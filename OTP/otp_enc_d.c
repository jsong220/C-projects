...
//recieves texts from otp_enc.c and returns randomized ciphertext
	//encryption of text done here
...
switch(spawnPid){
	...
	case 0: {
			...
			//while (buffer[bufLen] != '@'); // Keep reading until we find the @ terminating char.
			...
			//printf("buffer str size %li\n", strlen(buffer));

			//works cited - https://stackoverflow.com/questions/9578611/split-c-string-into-tokens-using-sscanf
			char plainText[80000];
			memset(plainText, '\0', 80000);
			char keyText[80000];
			memset(keyText, '\0', 80000);
			char type[10];
			memset(keyText, '\0', 10);
			int numOfLetters = 0;
			int numOfKeys = 0;

			sscanf(buffer,"%[^<]<%[^<]<%s", plainText, keyText,type);

			numOfLetters = (int) strlen(plainText);
			...
			for (i=0; i < numOfLetters; i++){
				...
				cipherText[i] = (plainText[i] + keyText[i]) % 27;

				cipherText[i] = cipherText[i] + 65;
				if((int)cipherText[i] == 91){
					cipherText[i] = ' ';
				}
				cipherText[numOfLetters] = '\0';
			//printf("help me first in loop..%d", strlen(cipherText));
			}
